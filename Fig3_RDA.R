...
alpinePlot <- ggplot(data=df.sites, aes(x=RDA1, y=RDA2 )) +  # , color= Region
  ...
  geom_segment(data=df.env, aes(x=xOrg, y=yOrg, xend=RDA1, yend=RDA2), size=3,
               #Change colours from  to: "firebrick1", "blue",  scale_color_gradient(low = "blue", high =  "firebrick1")+  #Colour pattern consistent with Fig3 and Fig2
               colour= c("firebrick1", "blue", "blue","firebrick1","darkolivegreen", "blue","firebrick1" ), arrow=arrow(length=unit(7,"point"),type = "open") )+
  
  annotate("text", x = -0.1, y = 0.2, label = c("Epacris petrophylla"), size=5, color="firebrick1",fontface="italic")+
  annotate("text", x = -0.1, y = 0.15, label = c("Nematolepis ovatifolia"), size=5, color="firebrick1",fontface="italic") + 
  annotate("text", x = -0.1, y = 0.1, label = c("Ozothamnus alpina"), size=5, color="firebrick1",fontface="italic") + 
  annotate("text", x = 0.15, y = -0.2, label = c("Orites lancifolius"), size=5, color="blue",fontface="italic") + 
  annotate("text", x = 0.15, y = -0.15, label = c("Grevillea australis"), size=5, color="blue",fontface="italic") + 
  annotate("text", x = 0.15, y = -0.1, label = c("Hovea montana"), size=5, color="blue",fontface="italic") +
...
alpinePlot     
ggsave(alpinePlot, width = 12, height = 7, file = "Fig5_AlpineRDA_SharpEdges.png")
...
#Solution to plotting errors (sometimes) ======
...
install.packages('ggord') # Install ggord
