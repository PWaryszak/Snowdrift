...
Six_height_cm
#write.table(Six_height_cm, file = "6TargetSgrubsTraits.txt", sep = ",", quote = FALSE, row.names = F)  #Save as table.txt and then copy into Word, 
...
s1
ggsave(s1, dpi=300, width = 2401, height = 2037, units = "px", filename = "Table1_PlotNewUpdated3_Regions2.jpg")



#Region as shape:
#Panel Plot of Table 1:
#Relevel to clup BHP/KNP species together
levels(as.factor(Six_height_cm$shrub_genus))


s1 <- ggplot(Six_height_cm, aes( x =  "", y=mean_height_cm)) +
  geom_point(aes(size = as.numeric(mean_area_cm3), fill=mean_LAI,shape=Region), colour = "black")+
  ...
  theme_classic()+
  labs( x = "Target shrub", y = "Mean target shrub height (cm)",
         fill = "LAI:", shape = "Region: ",size = "Area:", )+
  
  guides(shape = guide_legend(override.aes = list(size = 5)))+  # Adjust the symbol size in legends
