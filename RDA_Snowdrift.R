...
snow_stats <- as.data.frame(snow_stats) #snow_stats hold extra grouping variablles info. This line removes it.
...
alpinePlot <- ggplot(data=df.sites, aes(x=RDA1, y=RDA2 )) +  # , color= Region
  ...
  geom_segment(data=df.env, aes(x=xOrg, y=yOrg, xend=RDA1, yend=RDA2), size=3,
               colour= c("red", "darkblue", "darkblue","red","darkgreen", "darkblue","red" ), arrow=arrow(length=unit(7,"point"),type = "open") )+
  ...
  annotate("text", x = 0.15, y = -0.2, label = c("Orites lanceolata"), size=5, color="darkblue",fontface="italic") + 
  annotate("text", x = 0.15, y = -0.15, label = c("Grevillea australis"), size=5, color="darkblue",fontface="italic") + 
  annotate("text", x = 0.15, y = -0.1, label = c("Hovea montana"), size=5, color="darkblue",fontface="italic") +
  annotate("text", x = 0.08, y = 0.2, label = c("Open grassy"), size=5, color="darkgreen",fontface="bold")+ 
...
#ggsave(alpinePlot, width = 12, height = 7, file = "AlpineRDA_GOOD02.png")
