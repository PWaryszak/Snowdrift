...
litter_model_scaled <- lmer(massloss_Perc ~  Elevation_scaled +burialdepth_cm_scaled + 
...
tab_model(litter_model_scaled , show.stat=T, show.se=T, show.ci = F , digits = 2)
plot_model(litter_model_scaled)

#ORIGINAL MODEL (Originally Different scales):
litter_model_orig <- lmer(massloss_Perc ~  Elevation +burialdepth_cm + 
    timeburied_days + SLA_mm2mg  +
    Elevation         * timeburied_days +
    Elevation         * SLA_mm2mg +
    timeburied_days * SLA_mm2mg +  (1|site), data = ShrubLitter_DATA)

tab_model(litter_model_orig , show.stat=T, show.se=T, show.ci = F , digits = 2, 
         show.icc=FALSE, show.dev = FALSE, show.re.var=T, show.ngroups =TRUE)
...
litter_model_scaled <- lmer(litterlost_g ~  Elevation_scaled +burialdepth_cm_scaled + 
    timeburied_days_scaled + SLA_mm2mg_scaled  +
    Elevation_scaled          * timeburied_days_scaled +
    Elevation_scaled         * SLA_mm2mg_scaled +
    timeburied_days_scaled * SLA_mm2mg_scaled +  (1|site), data = ShrubLitter_DATA)

tab_model(litter_model_scaled , show.stat=T, show.se=T, show.ci = F , digits = 7)


#ORIGINAL MODEL (orginal scales):
litter_model_orig <- lmer(litterlost_g ~  Elevation +burialdepth_cm + 
    timeburied_days + SLA_mm2mg  +
    Elevation         * timeburied_days +
    Elevation         * SLA_mm2mg +
    timeburied_days * SLA_mm2mg +  (1|site), data = ShrubLitter_DATA)

tab_model(litter_model_orig , show.stat=T, show.se=T, show.ci = F , digits = 7)
